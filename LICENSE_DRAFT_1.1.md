Total Reciprocity Public License (TRPL)
Version: Draft 1.1 (Work in Progress)
Preamble
The intent of the Total Reciprocity Public License (TRPL) is to maximize the freedom of software architecture. Unlike permissions-based licenses, this license conditions the use of the software on the reciprocity of freedom: if you derive functional value from this code, the systems facilitating that value must also be free. This License ensures that any system deriving functional value from this software contributes back to the commons.
1. Definitions
...
"Intimate Communication" refers to an interaction where the Program and another work are functionally dependent on each other to perform a primary purpose. This is defined strictly as:
Static or dynamic linking.
Shared memory or complex Inter-Process Communication (IPC) where data structures are shared.
API calls (REST, RPC, GraphQL) where the calling software exposes the Program's specific logic or data model directly to a user or another system.
...
"Deployment" means the execution of the Program to provide a service, function, or output for the benefit of an organization, entity, or multiple users.
Personal Use Exception: Installation and execution of the Program on a personal device for the sole purpose of study, personal experimentation, or non-commercial content creation does not constitute Deployment.
...
All users interacting with the Program remotely.
All individuals within your organization who use the Program.
The general public, via a standard network repository.
...
Exception for Standard Tools and System Libraries:
The requirement to release Source Code for the Combined Work does not apply to "System Libraries" or "Standard Tools" (such as operating systems, compilers, interpreters, standard API clients like Postman or cURL, or web browsers) that are used merely to host, execute, or interact with the Program, provided those tools are not specific to the Program and are widely available to the public.
However, if a proprietary software component is created specifically to wrap, extend, or hide the functionality of the Program (e.g., a proprietary microservice wrapper), that component remains subject to the full terms of this License.
...
You must license such Functional Output under this License.
...
Training: You must release the full Source Code of the model (including Weights and Training Data) if you Deploy the model. The resulting Weights are considered Functional Output.
Inference: You cannot keep the model proprietary if it relies on the Program for inference logic.
Generation: Raw media or text generated by the model is considered Content Output and belongs to the user, provided it does not contain Functional Output.
